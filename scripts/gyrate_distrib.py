...
#function description: 
  #discards {equilibration_time} picoseconds at the start of the trajectory
  #splits the remaining trajectory into two equal time intervals and plots radius of gyration (Rg) distribution (approximated using histograms) of each time interval  
  #calculates the percentage of overlap between the two distributions 

#parameters:
  #equilibration_time: amount of time (in picoseconds) that the user wants to discard at the start of the trajectory 
  #num_bins: number of histogram bins used to approximate density distribution
  #filename: name of the .xvg file that contains the radius of gyration at every recorded timeframe of trajectory

def Rg_distrib(equilibration_time, num_bins,filename): 
  ...
  for bin_index in range(num_bins): #loops through "bins" list
    #then loop through Rg1 and Rg2 to count the number of times a radius falls into each bin during each of the two time intervals
...
num_bins = 50
filename = "18res_glycan_gyrate.xvg"
Rg_distrib(equilibration_time,num_bins,filename)
